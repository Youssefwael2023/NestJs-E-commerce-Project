...
function LowStock() {
  ...
  const handleSaveClick = async (id) => {
    ...
    try {
      ...
      setProducts(
        products
          .filter((product) => (product._id === id ? editedStock <= 10 : true))
          .map((product) =>
  ...
  return (
    <div className="container" style={{ minHeight: "100vh" }}>
      <h1 className="text-center pt-5 mb-5 text-light">Low Stock Products</h1>
      {alert.message && (
        <div
      ...
      {products.length === 0 && (
        <p className="text-center  fs-3">No low stock products found.</p>
      )}
      {products.length > 0 && (
...
export default withAdminAuth(LowStock);
